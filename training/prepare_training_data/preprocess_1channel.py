...
from lst_ai.register import rigid_reg, apply_warp_label
...
if __name__ == "__main__":
    ...
    assert str(args.flair).endswith(".nii.gz"), 'Please provide FLAIR image as a zipped nifti.'
    ...
    # An existing segmentation is read in image space by the same tools, so it
    # has to be harmonized the same way or it would disagree with the harmonized image.
    ...
    ## register FLAIR image to MNI space
    rigid_reg(moving = path_orig_flair,
              fixed = t1w_atlas,
              affine = path_affine_mni_flair,
              destination = path_mni_flair,
              n_threads = args.threads)


    ## apply HD-BET skull stripping to FLAIR image
    if args.fast:
        run_hdbet(input_image=path_mni_flair, output_image=path_mni_stripped_flair, device=args.device, mode="fast")
    else:
        run_hdbet(input_image=path_mni_flair, output_image=path_mni_stripped_flair, device=args.device, mode="accurate")

    # move processed mask to correct naming convention
    hdbet_mask = path_mni_stripped_flair.replace(".nii.gz", "_bet.nii.gz")
    shutil.move(hdbet_mask, path_mni_brainmask)

    # copy the stripped FLAIR image to the output directory
